...
依存は`fixedpoint`と`nco`です。phase/phase stepは`gndless_nco::Phase`、波形出力は現行どおりQ8.24 raw signed値です。ROM coreは1 clock latency、純粋なwave coreは入力phaseに対して組み合わせ、oscillator wrapperは`gndless_nco::Phasor`の同期resetと`phase_rst`を使います。振幅、duty、noise seed/stateの契約は各module doc commentを正とします。
